...
export let onGraphCreate = null;
export const setGraphCreateCallback = (cb) => { onGraphCreate = cb; };
export function useAIChatLogic() {
    ...
    const executeActionsClientSide = async (actions, originalText) => {
        if (!haWebSocket) {
        ...
        for (let i = 0; i < actions.length; i++) {
            ...
            if (action.service === 'recipe.timer' || action.service === 'timer.start' || action.service === 'timer.set') {
                ...
                try {
                    ...
                    const TimerPlugin = registerPlugin('TimerPlugin');
    ...
    const playNativeTTS = async (text, language, autoListen) => {
        try {
    ...
    const sendMessage = async (text, options) => {
        ...
        try {
            ...
            const mcpProxyUrl = activeConnection.mcp_proxy_url || undefined;
            ...
            if (options?.forcedLocation) {
                ...
                try {
                    ...
                    const position = await Promise.race([getLocationPromise, timeoutPromise]);
            ...
            const payload = {
                ...
                ha_url: activeConnection.api_url || '',
                ha_token: activeConnection.token || activeConnection.api_token || '',
            ...
            logger.info("🔑 HA Credentials Debug:", {
                ...
                api_url_exists: !!activeConnection.api_url,
                token_exists: !!activeConnection.token,
                api_token_exists: !!activeConnection.api_token
            });
