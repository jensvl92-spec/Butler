...
export function ButlerSuggestions() {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            setActiveSuggestion(payload.new); // Type cast for now
    ...
    const handleAction = async (accepted) => {
        ...
        if (accepted) {
            ...
            const hasComplexActions = activeSuggestion.actions.some((a) => a.type === 'create_automation') || (activeSuggestion.scheduled_actions && activeSuggestion.scheduled_actions.length > 0);
    ...
    return (_jsxs("div", { style: {
            ...
            border: '1px solid #374151'
        }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }, children: [_jsxs("h3", { style: { margin: 0, fontSize: '1.1rem', fontWeight: 600 }, children: ["\uD83D\uDCA1 ", activeSuggestion.title] }), _jsx("button", { onClick: () => handleAction(false), style: { background: 'transparent', border: 'none', color: '#9ca3af', cursor: 'pointer', fontSize: '1.2rem' }, children: "\u2715" })] }), _jsxs("p", { style: { margin: '0 0 16px 0', color: '#d1d5db', fontSize: '0.9rem' }, children: [activeSuggestion.description, activeSuggestion.scheduled_actions && activeSuggestion.scheduled_actions.length > 0 && (_jsxs("span", { style: { display: 'block', marginTop: '8px', color: '#60a5fa', fontWeight: 500 }, children: ["\u23F3 Scheduled: ", activeSuggestion.scheduled_actions[0].title] }))] }), _jsxs("div", { style: { display: 'flex', gap: '12px' }, children: [_jsx("button", { onClick: () => handleAction(false), style: {
                            flex: 1,
                            padding: '8px',
                            borderRadius: '6px',
                            ...
                            color: 'white',
                            cursor: 'pointer'
                        }, children: "No thanks" }), _jsx("button", { onClick: () => handleAction(true), style: {
                            flex: 1,
                            padding: '8px',
                            borderRadius: '6px',
                            border: 'none',
