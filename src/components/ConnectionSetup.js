...
export function ConnectionSetup({ onConnectionAdded }) {
    ...
    return (_jsxs("div", { className: "setup-container", children: [_jsxs("form", { onSubmit: handleSubmit, children: [_jsx("input", { type: "text", placeholder: "Connection Name", value: name, onChange: (e) => setName(e.target.value), required: true }), _jsx("input", { type: "url", placeholder: "Home Assistant URL (e.g., http://192.168.1.100:8123)", value: apiUrl, onChange: (e) => setApiUrl(e.target.value), required: true }), apiUrl.includes('.local') && (_jsxs("p", { style: { color: 'orange', fontSize: '0.8rem', marginTop: '4px' }, children: ["\u26A0\uFE0F Android often fails to resolve ", _jsx("b", { children: ".local" }), " domains. Please use the ", _jsx("b", { children: "IP Address" }), " (e.g. 192.168.1.x) if connection fails."] })), _jsx("input", { type: "password", placeholder: "Long-Lived Access Token", value: apiToken, onChange: (e) => setApiToken(e.target.value), required: true }), _jsx("button", { type: "submit", disabled: loading, children: loading ? 'Connecting...' : 'Connect' })] }), error && _jsx("p", { className: "error", children: error })] }));
}
