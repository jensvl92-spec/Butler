...
export async function getHAStates(url, token) {
...
export async function getHAServices(url, token) {
    const effectiveToken = getToken(token);
    ...
    if (Array.isArray(servicesArray)) {
        servicesArray.forEach((domainObj) => {
...
export async function callHAService(domain, service, serviceData, url, token) {
    const effectiveToken = getToken(token);
...
export function buildHAToolContext(devices, services) {
    ...
    const serviceList = Object.entries(services)
        ...
        return `${domain}: ${servicesArray.map((s) => s.service || s).join(', ')}`;
    })
        .join('\n');
